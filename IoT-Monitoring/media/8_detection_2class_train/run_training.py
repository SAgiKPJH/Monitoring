...
# ..\.venv\Scripts\python.exe run_training.py --dataset_path ..\\8_detection_2class_train\\dataset --params src\\params_gpu.json

"""OD_Training_Standard 2클래스(baby, baby_face) 학습 진입점 얇은 래퍼.

사용법:
    python run_training.py [--dataset_path ..\\8_detection_2class_train\\dataset] [--params src\\params_gpu.json]
기본 데이터셋은 ..\\7_auto_labeling\\dataset (images/ labels/ + classes.txt), 기본 params 는 src\\params_gpu.json.
클래스 수(nc)는 dataset\\classes.txt 로 정해짐 — 여기선 baby(0), baby_face(1) 2클래스.
...
# 데이터셋 본체는 7_auto_labeling\dataset (auto_label 출력 + baby_face 라벨 + classes.txt)
DEFAULT_DATASET = str(Path(__file__).resolve().parent.parent / '7_auto_labeling' / 'dataset')


def main() -> int:
    ap = argparse.ArgumentParser(description='baby+baby_face 2클래스 OD 학습 실행 (OD_Training_Standard 호출)')
